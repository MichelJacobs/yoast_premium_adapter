...
// 	jQuery("#wpbody-content").append(document.getElementById('wordpress_wakachi_widget'));
// })
...
function wordpress_wakachi_widget_join() {
	...
	var focuskw = jQuery('#yoast_wpseo_focuskw_text_input').val();
	
	var executed = false;
	
	if ( jQuery('#wordpress_wakachi_enable_title').is(":checked") ) {
		var domTitle = jQuery('#name');
	...
	if ( jQuery('#wordpress_wakachi_enable_content').is(":checked") ) {
			...
			var domContent = jQuery( '#description' );
			
			// テキストモード判定
			var isVisible = domContent.is(":visible");
			if (! isVisible) {
				jQuery('#description-html').click();
			}
